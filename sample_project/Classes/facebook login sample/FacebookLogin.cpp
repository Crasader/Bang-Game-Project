...
//  HelloCpp-mobile
//
//  Created by Guan Ying Chen on 2019/3/30.
//
...
bool HelloWorld::init()
{
    ...
    firebase::auth::Auth* auth = firebase::auth::Auth::Auth::GetAuth(firebase::App::GetInstance());
  ...
  //  _inviteText = "SDKBox (www.sdkbox.com)";
    ...
    */
    ...
    menu->setPosition(Vec2(winsize.width - labelSize.width / 2 - 16,labelSize.height / 2 + 16));
    addChild(menu);
    ...
    std::string FBAccessToken = sdkbox::PluginFacebook::getAccessToken().c_str();
    CCUserDefault::sharedUserDefault()->setStringForKey("AccessToken",FBAccessToken);   //Save accesstoken to cocos data.
    
    firebase::auth::Credential credential = firebase::auth::FacebookAuthProvider::GetCredential(FBAccessToken.c_str());
    
    firebase::Future<firebase::auth::User*> FB_sign_in_future = auth->SignInWithCredential(credential);
    if(FB_sign_in_future.status() == firebase::kFutureStatusComplete){
        INFO("log in firebase yes.");
    }
    else{
        INFO("log in firebase no.");
    }
...
void HelloWorld::createTestMenu()
{
    ...
    // ui
    {
        ...
        Label *userLabel = Label::createWithSystemFont(USERID, "sans", 32);
...
void HelloWorld::update(float delta) {
    ...
    if ( FB_sign_in_future_.status() == firebase::kFutureStatusComplete) {
        const AuthError error = static_cast<AuthError>(FB_sign_in_future_.error());
        if (error == firebase::auth::kAuthErrorNone) {
            INFO("Signed in successfully.");
        } else {
            INFO("ERROR: Sign in failed: %d, `%s`", error,
                       FB_sign_in_future_.error_message());
            
        }
        FB_sign_in_future_.Release();
    }
}
